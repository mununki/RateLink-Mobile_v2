  ...
  return (
    <View>
      <View styleName="horizontal sm-gutter">
        ...
        />
        <Button styleName="secondary sm-gutter stretch" style={{ flex: 0.1 }} onPress={() => handleSearch()}>
          <Icon name="search" />
        </Button>
      </View>
    </View>
...
const SearchResult = ({ foundUsers, onlyTellers }) => {
  return (
    <View>
      <Text styleName="md-gutter-left">{foundUsers.length} matches found</Text>
      {foundUsers.map(user => (
        <UserCard key={user.id} user={user} isTeller={true} onlyTellers={onlyTellers} />
      ))}
    </View>
  );
};
  ...
  _handleSearch = () => {
    ...
    this.props.client
      ...
      .then(res => {
        ...
        Toast.show(`${res.data.findUsers.length} matches found`, Toast.SHORT);
      })
      .catch(error => Toast.show("Please try again.", Toast.SHORT));
  };
